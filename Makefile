...
build:
	docker run --rm -i -w=/var/node -u=$(id -u ${USER}):$(id -g ${USER}) -p 3000:3000 -v "${ROOT}/${NODE_PACKAGE_JSON_PATH}:/var/node/:cached,rw" ${NODE_DOCKER_IMAGE} npm run build -- --host 0.0.0.0
